...
辞書モデルで使用してるデータは，上記「使用データ」のサイトからダウンロードし，スクリプトを利用し作成することができる。詳細については，[dict](./dict/README.md)を参照のこと。
...
## 実行方法
本モデルについては，コマンドラインから実行するプログラムとして提供している。また，[氏名漢字カナ突合API](https://api.kktg.digital.go.jp/)の内部でも利用している。
...
AIモデルをPythonのPytorchで作成し，Python及びJavaで推計するサンプルを提供している。詳細は[sample](./sample/README.md)を参照のこと。
