...
class KanjiKanaTransformer:
    ...
    def load_by_vocab(self, src_vocab, tgt_vocab, params):
        ...
        for s in src_vocab:
            src[s]=1
        #self.vocab_transform[params["source_lang"]]= vocab(src,min_freq=0,specials=SPECIAL_SYMBOLS,special_first=True)
        self.vocab_transform[params["source_lang"]] = Vocab(split_tokenizer, src, special_tokens=SPECIAL_SYMBOLS, unk_token='<unk>')
        tgt=OrderedDict()
        for s in tgt_vocab:
            tgt[s]=1
        #self.vocab_transform[params["target_lang"]]= vocab(tgt,min_freq=0,specials=SPECIAL_SYMBOLS, special_first=True)
        self.vocab_transform[params["target_lang"]] = Vocab(split_tokenizer, src, special_tokens=SPECIAL_SYMBOLS, unk_token='<unk>')
